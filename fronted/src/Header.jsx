      ...
      <header className="flex justify-between">
        ...
        {!(location.hash == "#/login") && !(location.hash == "#/register") && (
          <nav className="w-full flex justify-center mt-8 gap-2 mb-8">
            <Link className={linkClasses("account")} to={"/trading/account"}>
              Account
            </Link>
            <Link
              className={linkClasses("optionCopyTrading")}
              to={"/trading/optionCopyTrading"}
            >
              Option Trading
            </Link>
            <Link
              className={linkClasses("stockCopyTrading")}
              to={"/trading/stockCopyTrading"}
            >
              Stock Trading
            </Link>
            <Link
              className={linkClasses("optionTradeHistory")}
              to={"/trading/optionTradeHistory"}
            >
              Option History
            </Link>
            <Link
              className={linkClasses("stockTradeHistory")}
              to={"/trading/stockTradeHistory"}
            >
              Stock History
            </Link>
          </nav>
        ...
        <div className="flex items-center gap-3 py-1 px-5">
          ...
          {!(location.hash == "#/login") &&
            !(location.hash == "#/register") && (
              <button
                onClick={toggleLogoutConfirmationOverlay}
                className="primary max-w-sm mt-2 whitespace-nowrap"
              >
                Log Out
              </button>
            )}
        ...
        <Overlay isOpen={isLogoutConfirmation}>
          <LogoutConfirmation
            onClose={toggleLogoutConfirmationOverlay}
          ></LogoutConfirmation>
        </Overlay>
      </header>
    </div>

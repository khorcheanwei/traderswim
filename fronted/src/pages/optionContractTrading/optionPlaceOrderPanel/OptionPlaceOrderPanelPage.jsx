...
export default function OptionPlaceOrderPanelPage({ callOption }) {
  ...
  for (let index = 0; index < copyTradingOrderMainData.length; index++) {
    let optionChainSymbol =
      copyTradingOrderMainData[index]["optionChainSymbol"];
    let optionChainInstruction =
      copyTradingOrderMainData[index]["optionChainInstruction"];
    let optionChainDescription =
      copyTradingOrderMainData[index]["optionChainDescription"];
  ...
  optionPlaceOrderPanelData.sort((a, b) =>
    a.optionChainSymbol.localeCompare(b.optionChainSymbol),
  );
  var data = React.useMemo(
    () => optionPlaceOrderPanelData,
    [optionPlaceOrderPanelData],
  );

  return <OptionPlaceOrderPanelTable columns={columns} data={data} />;
}

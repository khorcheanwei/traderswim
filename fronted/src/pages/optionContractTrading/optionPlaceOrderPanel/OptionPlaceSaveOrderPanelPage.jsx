...
import OptionPlaceSaveOrderPanelTable, {
  SettingsPanel,
} from "./OptionPlaceSaveOrderPanelTable";
...
export default function OptionPlaceSaveOrderPanelPage({ callOption }) {
  ...
  columns_list = [
    ...
    },
    {
      ...
      Cell: (row) => (
        <SettingsPanel
          row={row}
          setOptionContractSaveOrderList={setOptionContractSaveOrderList}
        />
      ),
    },
  ];
  ...
  for (let index = 0; index < optionContractSaveOrderList.length; index++) {
    let optionChainSymbol =
      optionContractSaveOrderList[index]["optionChainSymbol"];
    let optionChainInstruction =
      optionContractSaveOrderList[index]["optionChainInstruction"];
    let optionChainDescription =
      optionContractSaveOrderList[index]["optionChainDescription"];

    if (
      !optionChainSymbolSet.has(optionChainSymbol) &&
      optionChainInstruction == "BUY_TO_OPEN"
    ) {
  ...
  optionPlaceSaveOrderPanelData.sort((a, b) =>
    a.optionChainSymbol.localeCompare(b.optionChainSymbol),
  );
  var data = React.useMemo(
    () => optionPlaceSaveOrderPanelData,
    [optionPlaceSaveOrderPanelData],
  );
  ...
  async function option_contract_save_order_list_fetch() {
    try {
      const { data } = await axios.get(
        "/option_contract_save_order/get_option_contract_save_order_list",
      );
  ...
  return <OptionPlaceSaveOrderPanelTable columns={columns} data={data} />;
}

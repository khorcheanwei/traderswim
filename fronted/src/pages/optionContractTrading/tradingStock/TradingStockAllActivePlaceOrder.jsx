...
export default function TradingStockAllActivePlaceOrder({ onClose }) {
  ...
  var optionChainOrderTypeList = ["LIMIT", "MARKET"];

  const { optionContractSaveOrderList, setOptionContractSaveOrderList } =
    useContext(OptionPlaceOrderPanelContext);
  ...
  const [refreshOptionChainStrikeListKey, setRefreshOptionChainStrikeListKey] =
    useState(0);
  ...
  async function handlePlaceOrder() {
    ...
    try {
      ...
      const { data } = await axios.post("/copy_trading_account/place_order/", {
    ...
    try {
      if (
        ...
        const { data } = await axios.post(
          "/option_contract_save_order/add_option_contract_save_order/",
          {
            optionChainSymbol,
            optionChainDescription,
            optionChainInstruction,
  ...
  async function getOptionChainList() {
    try {
      ...
      const { data } = await axios.get(
        "/copy_trading_account/get_option_chain_list/",
        {
      ...
      if (data != null) {
        ...
        const firstOptionChainSymbol =
          data[firstOptionChainDate][firstOptionChainStrikeList[0]][0][
            "symbol"
          ];
        const firstOptionChainBid =
          data[firstOptionChainDate][firstOptionChainStrikeList[0]][0]["bid"];
  ...
  useEffect(() => {
    if (
      (!isOptionChainCall && isOptionChainPut) ||
      (isOptionChainCall && !isOptionChainPut)
    ) {
      getOptionChainList();
    }
        ...
        <div className="flex gap-10">
          <div className="flex flex-col justify-between">
            <div>
              <div className="mb-4">
                ...
                <div className="flex gap-5">
                  <input
                    ...
                    onInput={(event) =>
                      setOptionContractTicker(event.target.value.toUpperCase())
                    }
                  />
                  <button
              ...
              <div className="overflow-scroll">
                {optionContractTickerList.map(
                  (currentOptionContractTicker, index) => (
                    <div key={index} className="option contracts">
                      <div className="flex justify-between gap-5">
                        <button
                          ...
                          onClick={() =>
                            handleSetStockName(currentOptionContractTicker)
                          }
                        >
                          {currentOptionContractTicker}
                        </button>
          ...
          <div>
            ...
            <div>
                ...
                <div className="grid items-end gap-6 mb-6 grid-cols-2">
                  <div className="relative">
                    <input
                      ...
                      type="text"
                      onChange={(event) =>
                        setOptionContractTickerName(event.target.value)
                      }
              ...
              <div className="relative">
                ...
                >
                  {optionChainDateList.map((option_chain_date, index) => (
                    <option key={index}>
                      {getOptionChainRealDate(option_chain_date)}
                    </option>
                  ))}
                </select>
                <label
                  className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-1 peer-focus:scale-75 peer-focus:-translate-y-3 left-1"
                  htmlFor="option_chain_date_list"
                >
              ...
              <div className="relative">
                <AutocompleteList
                  key={refreshOptionChainStrikeListKey}
                  list={optionChainStrikeList}
                  onData={getOptionChainBidPrice}
                ></AutocompleteList>
              ...
              <div className="grid items-end gap-6 mb-6 md:grid-cols-2">
                <div className="relative">
                  <select
                    id="option_chain_instruction"
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                    value={optionChainInstruction}
                    onChange={(event) =>
                      setOptionChainInstruction(event.target.value)
                    }
                  >
                  ...
                  <label
                    className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-1 peer-focus:scale-75 peer-focus:-translate-y-3 left-1"
                    htmlFor="option_chain_instruction"
                  >
                ...
                <div className="relative">
                  <select
                    id="option_chain_order_type"
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
                    value={optionChainOrderType}
                    onChange={(event) =>
                      setOptionChainOrderType(event.target.value)
                    }
                  >
                  ...
                  <label
                    className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-3 scale-75 top-1 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-1 peer-focus:scale-75 peer-focus:-translate-y-3 left-1"
                    htmlFor="option_chain_order_type"
                  >
              ...
              <div className="grid items-end gap-6 mb-6 grid-cols-2">
                <div className="relative">
                  <input
                    id="option_contract_total"
                    className="block px-2.5 pb-1.5 pt-3 w-full text-sm text-gray-900 bg-transparent  border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    type="text"
                    onChange={(event) =>
                      setOptionChainQuantity(event.target.value)
                    }
                ...
                {optionChainOrderType == "LIMIT" && (
                  <div className="relative">
                    <input
                      ...
                      type="text"
                      onChange={(event) =>
                        setOptionChainPrice(event.target.value)
                      }

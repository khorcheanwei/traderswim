...
import {
  TextOptionChainDescriptionColorPanel,
  TextOptionChainFilledQuantityColorPanel,
  TextOptionChainPriceColorPanel,
  TextOptionChainQuantityColorPanel,
  TextOptionChainInstructionColorPanel,
  TextOptionChainStatusColorPanel,
  TextOptionChainOrderTypeColorPanel,
  TextOptionChainEnteredTimeColorPanel,
  TextAccountNameColorPanel,
  TextAccountUsernameColorPanel,
  ChangeOrderIndividualPanel,
  MakeSelectedOrderPanel,
} from "./CopyTradingAllAccountOrderTable";

export default function CopyTradingAllAccountOrderPage({
  rowCopyTradingOrder,
  onClose,
}) {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: (row) => (
          <MakeSelectedOrderPanel
            row={row}
            setSelectedOrderDict={setSelectedOrderDict}
          />
        ),
      },
    ],
  ...
  const agentTradingSessionID =
    rowCopyTradingOrder.row.original.agentTradingSessionID;
  ...
  var data = React.useMemo(
    () => copyTradingAllAccountData,
    [copyTradingAllAccountData],
  );

  // replace and cancel selected orders
  const [isOpenOrderDeleteSelected, setIsOpenOrderDeleteSelected] =
    useState(false);
  const [isOpenOrderReplaceSelected, setIsOpenOrderReplaceSelected] =
    useState(false);
  const [
    isOpenWarningMessageOrderSelected,
    setIsOpenWarningMessageOrderSelected,
  ] = useState(false);
    ...
    <div>
      <div className="flex justify-between">
        ...
        <div className="flex space-x-2 mr-10">
          <div
            onClick={orderReplaceSelectedClose}
          ...
          <div
            onClick={orderDeleteSelectedClose}
      ...
      <div>
        <Overlay isOpen={isOpenWarningMessageOrderSelected}>
          <TradingStockWarningMessage
            warningMessage={"Please select at least one order!"}
            onClose={warningMessageOrderSelectedClose}
          ></TradingStockWarningMessage>
        </Overlay>
        <Overlay isOpen={isOpenOrderReplaceSelected}>
          <TradingStockReplaceOrderSelected
            rowCopyTradingOrderSelected={rowCopyTradingOrder}
            selectedOrderDict={selectedOrderDict}
            onClose={orderReplaceSelectedClose}
          ></TradingStockReplaceOrderSelected>
        </Overlay>
        <Overlay isOpen={isOpenOrderDeleteSelected}>
          <TradingStockDeleteOrderSelected
            rowCopyTradingOrderSelected={rowCopyTradingOrder}
            selectedOrderDict={selectedOrderDict}
            onClose={orderDeleteSelectedClose}
          ></TradingStockDeleteOrderSelected>
        </Overlay>
      </div>

...
export default function CopyTradingAllAccountPositionPage({
    ...
    () => [
      ...
      },
      {
        ...
        Cell: (row) => (
          <MakeSelectedPositionPanel
            row={row}
            setSelectedPositionDict={setSelectedPositionDict}
          />
        ),
      },
    ],
  ...
  const optionChainDescription =
    rowCopyTradingPosition.row.original.optionChainDescription;

  const [isOpenPositionExitSelected, setIsOpenPositionExitSelected] =
    useState(false);
  const [warningMessage, setWarningMessage] = useState("");
  const [
    isOpenWarningMessagePositionSelected,
    setIsOpenWarningMessagePositionSelected,
  ] = useState(false);
  ...
  const positionExitSelectedClose = async () => {
    if (Object.keys(selectedPositionDict).length == 0) {
      ...
      for (const accountUsername in selectedPositionDict) {
        ...
        optionChainSettledQuantitySet.add(
          selectedPosition["optionChainSettledQuantity"],
        );
      }
  ...
  const copyTradingPositionAllAccountData =
    copyTradingPositionDataDict[optionChainDescription];
  var data = React.useMemo(
    () => copyTradingPositionAllAccountData,
    [copyTradingPositionAllAccountData],
  );
    ...
    <div>
      <div className="flex justify-between">
        ...
        <div className="flex space-x-2 mr-10">
          <div
            onClick={positionExitSelectedClose}
      ...
      <Overlay isOpen={isOpenWarningMessagePositionSelected}>
        <TradingStockWarningMessage
          ...
          onClose={warningMessagePositionSelectedClose}
        ></TradingStockWarningMessage>
      </Overlay>
      <Overlay isOpen={isOpenPositionExitSelected}>
        <TradingStockExitOrderSelected
          selectedPositionDict={selectedPositionDict}
          onClose={positionExitSelectedClose}
        ></TradingStockExitOrderSelected>
      </Overlay>

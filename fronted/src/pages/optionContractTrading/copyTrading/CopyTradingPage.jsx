...
export default function CopyTradingPage({ children }) {
  ...
  async function fetchCopyTradingAccountData() {
    try {
      ...
      if (
        copyTradingAccountDataDictResponse != null &&
        copyTradingAccountDataDictResponse.length != 0
      ) {
        ...
        for (const [key, value] of Object.entries(
          ...
          for (let index = 0; index < value.length; index++) {
            ...
            let currentOptionChainFilledQuantity =
              value[index]["optionChainFilledQuantity"];

            optionChainStatusSet.add(currentOptionChainStatus);
            optionChainFilledQuantitySet.add(currentOptionChainFilledQuantity);

            if (
              copyTradingOptionChainStatusColor &&
              !optionChainStatusInactiveList.includes(currentOptionChainStatus)
            ) {
          ...
          if (
            optionChainStatusSet.size > 1 ||
            optionChainFilledQuantitySet.size > 1
          ) {
      ...
      if (
        copyTradingPositionDataDictResponse != null &&
        copyTradingPositionDataDictResponse.length != 0
      ) {

...
function GlobalFilter({
  ...
  const { isOpenAccountLogin, setIsOpenAccountLogin } =
  ...
  return (
    <div className="w-full">
      <div className="flex justify-end">
        <Button className="text-gray-700 " onClick={accountAddClose}>
          ADD ACCOUNT
        </Button>
      </div>
      <div>
...
export function TradingActiveToggle(row) {
  ...
  const [accountTradingActive, setAccountTradingActive] = useState(
    row.cell.row.original.accountTradingActive,
  );

  async function updateTradingActive(
    accountUsername,
    accountTradingActiveUpdated,
  ) {
    try {
      // trigger connection ON/OFF to account trading session
      await axios.post("/trading_account/trading_active", {
        accountUsername: accountUsername,
        accountTradingActive: accountTradingActiveUpdated,
      });
  ...
  return (
    <label className="relative inline-flex content-center">
      <input
        ...
        checked={accountTradingActive}
        onChange={handleTradingActiveChange}
      ></input>
    ...
    <div className="flex">
      ...
      <Overlay isOpen={isShowAccountInfo}>
        <AccountInfo
          ...
          onClose={accountInfoClose}
        ></AccountInfo>
      </Overlay>
      <Overlay isOpen={isOpenAccountDelete}>
        <AccountDeleteConfirmation
          rowAccount={rowAccount}
          onClose={accountDeleteClose}
        ></AccountDeleteConfirmation>
      </Overlay>
    </div>
  );

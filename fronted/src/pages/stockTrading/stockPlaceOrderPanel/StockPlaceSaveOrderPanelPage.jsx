...
import StockPlaceSaveOrderPanelTable, {
  SettingsPanel,
} from "./StockPlaceSaveOrderPanelTable";
...
export default function StockPlaceSaveOrderPanelPage() {
  ...
  columns_list = [
    ...
    },
    {
      ...
      Cell: (row) => (
        <SettingsPanel
          row={row}
          setStockSaveOrderList={setStockSaveOrderList}
        />
      ),
    },
  ];
  ...
  stockPlaceSaveOrderPanelData.sort((a, b) =>
    a.stockSymbol.localeCompare(b.stockSymbol),
  );
  var data = React.useMemo(
    () => stockPlaceSaveOrderPanelData,
    [stockPlaceSaveOrderPanelData],
  );
  ...
  async function stock_contract_save_order_list_fetch() {
    try {
      const { data } = await axios.get(
        "/stock_save_order/get_stock_save_order_list",
      );
  ...
  return <StockPlaceSaveOrderPanelTable columns={columns} data={data} />;
}

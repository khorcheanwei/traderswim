...
export default function StockExitOrder({
  ...
  let rowStockPrice =
    rowCopyTradingPosition.cell.row.original.stockAveragePrice;
  ...
  async function handleExitOrder() {
    ...
    try {
      const allTradingAccountsOrderList =
        stockCopyTradingPositionAllAccountData.map((item) => ({
      ...
      const { data } = await axios.post("/stock_copy_trading/exit_order/", {

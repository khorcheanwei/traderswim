...
export default function StockAllActivePlaceOrder({ onClose }) {
    ...
    try {
      if (
        ...
        const { data } = await axios.post(
          "/stock_save_order/add_stock_save_order/",
          {
            stockSymbol,
            stockSession,
            stockDuration,
  ...
  async function handleStockRemove(currentStockTicker) {
    const response = await axios.delete("/stock/remove_stock/", {
      data: { stockSymbol: currentStockTicker },
    });
          ...
          <div>
            <div className="mb-4">
              <h1 className="block text-gray-700 text-lm font-bold mb-2">
                Stock Place Order On All Active Accounts
              </h1>
            </div>
            <StockHandleOrder

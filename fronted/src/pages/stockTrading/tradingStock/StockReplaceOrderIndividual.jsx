...
const StockReplaceOrderIndividual = memo(
  ({ rowCopyTradingOrderIndividual, onClose }) => {
    ...
    let agentTradingSessionID =
      rowCopyTradingOrderIndividual.cell.row.original.agentTradingSessionID;

    let accountId = rowCopyTradingOrderIndividual.cell.row.original.accountId;
    let accountName =
      rowCopyTradingOrderIndividual.cell.row.original.accountName;
    let accountUsername =
      rowCopyTradingOrderIndividual.cell.row.original.accountUsername;

    let stockOrderId =
      rowCopyTradingOrderIndividual.cell.row.original.stockOrderId;
    let rowStockSymbol =
      rowCopyTradingOrderIndividual.cell.row.original.stockSymbol;
    let rowStockSession =
      rowCopyTradingOrderIndividual.cell.row.original.stockSession;
    let rowStockDuration =
      rowCopyTradingOrderIndividual.cell.row.original.stockDuration;
    let rowStockOrderType =
      rowCopyTradingOrderIndividual.cell.row.original.stockOrderType;
    let rowStockInstruction =
      rowCopyTradingOrderIndividual.cell.row.original.stockInstruction;
    let rowStockPrice =
      rowCopyTradingOrderIndividual.cell.row.original.stockPrice ?? 0;
    let rowStockStopPrice =
      rowCopyTradingOrderIndividual.cell.row.original.stockStopPrice ?? 0;
    let rowStockStopPriceLinkType =
      rowCopyTradingOrderIndividual.cell.row.original.stockStopPriceLinkType ??
      "VALUE";
    let rowStockStopPriceOffset =
      rowCopyTradingOrderIndividual.cell.row.original.stockStopPriceOffset ??
      0.1;
    let rowStockQuantity =
      rowCopyTradingOrderIndividual.cell.row.original.stockQuantity;
    ...
    async function handleReplaceOrderIndividual() {
      ...
      try {
        ...
        const { data } = await axios.put(
          "/stock_copy_trading/replace_order_individual/",
          {
          ...
          <div>
            <div className="mb-4">
              <h1 className="block text-gray-700 text-lm font-bold mb-2">
                Stock Replace Order (Individual) - <b>{accountUsername}</b>
              </h1>
            </div>
            <StockHandleOrder
